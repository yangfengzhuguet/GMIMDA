...
class GCN(nn.Module):
    ...
    def forward(self, sim_set, flag_):
        if flag_ == 'miRNA':
            ...
            #                     sim_set['miRNA_mut']['mi_gua'][sim_set['miRNA_mut']['mi_gua_edges'][0], sim_set['miRNA_mut']['mi_gua_edges'][HMDD v2]]))
            # x_m_g2 = torch.relu(self.mi_SNF_2(x_m_g1, sim_set['miRNA_mut']['mi_gua_edges'].to(device), sim_set['miRNA_mut']['mi_gua']
            #                     [sim_set['miRNA_mut']['mi_gua_edges'][0], sim_set['miRNA_mut']['mi_gua_edges'][HMDD v2]]))
            #
            # x_m_c1 = torch.relu(self.mi_SNF_1(mi_infeats.to(device), sim_set['miRNA_mut']['mi_cos_edges'].to(device),
            #                     sim_set['miRNA_mut']['mi_cos'][sim_set['miRNA_mut']['mi_cos_edges'][0], sim_set['miRNA_mut']['mi_cos_edges'][HMDD v2]]))
            # x_m_c2 = torch.relu(self.mi_SNF_2(x_m_c1, sim_set['miRNA_mut']['mi_cos_edges'].to(device), sim_set['miRNA_mut']['mi_cos']
            #                     [sim_set['miRNA_mut']['mi_cos_edges'][0], sim_set['miRNA_mut']['mi_cos_edges'][HMDD v2]]))
            #
            # x_m_f1 = torch.relu(self.mi_SNF_1(mi_infeats.to(device), sim_set['miRNA_mut']['mi_fun_edges'].to(device),
            #                     sim_set['miRNA_mut']['mi_fun'][sim_set['miRNA_mut']['mi_fun_edges'][0], sim_set['miRNA_mut']['mi_fun_edges'][HMDD v2]]))
            # x_m_f2 = torch.relu(self.mi_SNF_2(x_m_f1, sim_set['miRNA_mut']['mi_fun_edges'].to(device), sim_set['miRNA_mut']['mi_fun']
            #                     [sim_set['miRNA_mut']['mi_fun_edges'][0], sim_set['miRNA_mut']['mi_fun_edges'][HMDD v2]]))
            ...
            #                     sim_set['disease_mut']['di_gua'][sim_set['disease_mut']['di_gua_edges'][0], sim_set['disease_mut']['di_gua_edges'][HMDD v2]]))
            # y_d_g2 = torch.relu(self.di_SNF_2(y_d_g1, sim_set['disease_mut']['di_gua_edges'].to(device), sim_set['disease_mut']['di_gua']
            #                     [sim_set['disease_mut']['di_gua_edges'][0], sim_set['disease_mut']['di_gua_edges'][HMDD v2]]))
            #
...
class GUET(nn.Module):
    ...
    def forward(self, sim_set, SVD_NMF, train_miRNA_index, train_disease_index):
        ...
        mi_final = 0.67 * best_miRNA_strategies + 0.33 * mi_NMF
        di_final = 0.67 * best_disease_strategies + 0.33 * di_NMF
