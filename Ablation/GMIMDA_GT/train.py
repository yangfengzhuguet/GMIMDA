...
def show_auc(pre_score, label, flag):
    ...
    #     plt.plot([0, HMDD v2], [0, HMDD v2], color='navy', lw=2, linestyle='--')
    #     plt.xlim([0.0, HMDD v2.0])
    #     plt.ylim([0.0, HMDD v2.05])
...
def train_GUET(arges, model, sim_set, SVD_NMF, optimizer, pair_pos_neg_fengceng, device):
    ...
    for epoch_ in tqdm.tqdm(range(arges.epoch), desc='Training Epochs'):
        ...
        #     #            loc='upper right', bbox_to_anchor=(HMDD v2.15, HMDD v2))
        #
        #     plt.title(f'epoch:{epoch_ + 1}', fontsize=20, y=-0.12)
        #     # plt.xlabel('t-SNE Dimension HMDD v2', fontsize=20)
        #     # plt.ylabel('t-SNE Dimension 2', fontsize=20)
        #
        #     plt.savefig(f"HMDDv2{epoch_ + 1}.png", dpi=600, bbox_inches='tight', pad_inches=0.1)
        #     plt.close()
    ...
    with (torch.no_grad()):
        ...
        # np.save(f'data/miR2Disease/f_tpr/fpr_10.npy', fp)
        # np.save(f'data/miR2Disease/f_tpr/tpr_10.npy', tp)
        # np.save(f'data/miR2Disease/p_r/p_10.npy', pre)
        # np.save(f'data/miR2Disease/p_r/r_10.npy', rec)
        # np.save(f'circRNA-disease_datasets/f_tpr/fpr_5.npy', fp)
        # np.save(f'circRNA-disease_datasets/f_tpr/tpr_5.npy', tp)
        # np.save(f'circRNA-disease_datasets/p_r/p_5.npy', pre)
        # np.save(f'circRNA-disease_datasets/p_r/r_5.npy', rec)
        # np.save(f'MLP/HMDD v4/f_tpr/fpr_0.npy', fp)
        # np.save(f'MLP/HMDD v4/f_tpr/tpr_0.npy', tp)
        # np.save(f'MLP/HMDD v4/p_r/p_0.npy', pre)
        # np.save(f'MLP/HMDD v4/p_r/r_0.npy', rec)
...
#     # fp, tp, thresholds_roc = roc_curve(test_label_fc, y_prob)
#     # pre, rec, thresholds_pr = precision_recall_curve(test_label_fc, y_prob)
#     # np.save(f'RF/HMDD v4/f_tpr/fpr_0.npy', fp)
#     # np.save(f'RF/HMDD v4/f_tpr/tpr_0.npy', tp)
#     # np.save(f'RF/HMDD v4/p_r/p_0.npy', pre)
#     # np.save(f'RF/HMDD v4/p_r/r_0.npy', rec)
#
#     # xgb
...
#     fp, tp, thresholds_roc = roc_curve(test_label_fc, y_prob)
#     pre, rec, thresholds_pr = precision_recall_curve(test_label_fc, y_prob)
#     np.save(f'XGB/HMDD v4/f_tpr/fpr_0.npy', fp)
#     np.save(f'XGB/HMDD v4/f_tpr/tpr_0.npy', tp)
#     np.save(f'XGB/HMDD v4/p_r/p_0.npy', pre)
#     np.save(f'XGB/HMDD v4/p_r/r_0.npy', rec)
#
