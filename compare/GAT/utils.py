...
def get_syn_sim (k1, k2):
    ...
    disease_semantic_sim = read_csv('data/HMDD v_4/disease_sim/disSSim.csv') # disease semantic similarity
    disease_GIP_sim = read_csv('data/HMDD v_4/disease_sim/disGIPSim.csv') # disease gip similarity
    disease_cos_sim = read_csv('data/HMDD v_4/disease_sim/disCosSim.csv') # disease cosine similarity

    miRNA_GIP_sim = read_csv('data/HMDD v_4/miRNA_sim/miFunSim_norm.csv') # miRNA function similarity
    miRNA_cos_sim = read_csv('data/HMDD v_4/miRNA_sim/miGIPSim.csv') # miRNA gip similarity
    miRNA_func_sim = read_csv('data/HMDD v_4/miRNA_sim/miCosSim.csv') # miRNA cosine similarity
...
def get_fold():
    ...
    for fold in range(num_cross_val):
        ...
        # # Replace -HMDD v2 with 0 in the third column of the train.
        # train[:, 2][train[:, 2] == '-HMDD v2'] = 0
        #
        # # Replace -HMDD v2 with 0 in the third column of the test.
        # test[:, 2][test[:, 2] == '-HMDD v2'] = 0

        # Upsetting the datasets again
        ...
        # Define the directory and file name where the datasets will be saved
        parent_dir = "data/miR2Disease/0-HMDD v2/10-folds_balance"
        ...
        # Saved files with 0: miRNA index, HMDD v2: disease index, 2: label, 3: useless
...
def get_fc():
    ...
    # Split the datasets into training and testing sets (4/5 for training, HMDD v2/5 for testing)
    train_ratio = 0.8
    test_ratio = 0.2
...
def get_data():
    ...
    mi_final = read_csv('data/HMDD v_4/miRNA_sim/mi_SNF.csv').to(device)
    mi_final_edges = get_edge_index(mi_final).to(device)

    di_final = read_csv('data/HMDD v_4/disease_sim/di_SNF.csv').to(device)
    ...
    # mi_gua = read_csv('datasets/miR2Disease/miRNA_sim/m_gs.csv').to(device)
    # mi_gua_edges = get_edge_index(mi_gua).to(device)
    # mi_cos = read_csv('datasets/miR2Disease/miRNA_sim/m_ss.csv').to(device)
    # mi_cos_edges = get_edge_index(mi_cos).to(device)
    # mi_fun = read_csv('datasets/miR2Disease/miRNA_sim/m_fs.csv').to(device)
    # mi_fun_edges = get_edge_index(mi_fun).to(device)
    #
    # di_gua = read_csv('datasets/miR2Disease/disease_sim/d_gs.csv').to(device)
    # di_gua_edges = get_edge_index(di_gua).to(device)
    # di_cos = read_csv('datasets/miR2Disease/disease_sim/d_ts.csv').to(device)
    # di_cos_edges = get_edge_index(di_cos).to(device)
    # di_sem = read_csv('datasets/miR2Disease/disease_sim/d_ss.csv').to(device)
...
#
# SVD_NMF = dict()
# mda = pd.read_csv('datasets/HMDD v_4/m_d.csv', header=None, sep=',').to_numpy()
#
