...
def load_networks(path):
    ...
    for file in path:
        ...
        network = RWR(network)
        # network = scale(network, axis=HMDD v2)
...
class datapro:
    ...
    #     pos_index_matrix = np.mat(np.where(m_d_matrix == HMDD v2))
    ...
    def datasplit(self, args, m_d_matrix):
        pos_indices = np.where(m_d_matrix == 1)
        neg_indices = np.where(m_d_matrix == 0)

        pos_pairs = np.column_stack(pos_indices)
        neg_pairs = np.column_stack(neg_indices)

        min_samples = min(len(pos_pairs), len(neg_pairs))
        samples_per_fold = min_samples // args.k_fold
        ...
        for i in range(args.k_fold):
            ...
            end = (i + 1) * samples_per_fold if i != args.k_fold - 1 else min_samples

            fold_pos = pos_pairs[start:end]
            fold_neg = neg_pairs[start:end]

            fold_indices = np.concatenate([fold_pos, fold_neg]).tolist()
